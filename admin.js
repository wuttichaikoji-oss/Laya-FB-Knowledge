...
function latestActivity(user = {}) {
  const candidates = [
    ...
    tsToDate(user.lastQuizAt),
    tsToDate(user.createdAt)
  ].filter(Boolean);
...
function strengthSummary(user, attempts = []) {
  ...
  attempts.forEach(attempt => {
    const stats = attempt.categoryStats || Object.fromEntries(((attempt.categoryBreakdown || [])).map(row => [row.category || 'General', { correct: Number(row.correct || 0), total: Number(row.total || 0) }]));
...
function renderAttempts() {
  ...
  state.attempts
    ...
    .forEach(item => {
      ...
      const scopeText = item.scopeLabel || item.scope || item.category || item.sourceMode || 'Quiz';
      const weaknesses = Array.isArray(item.weaknesses) ? item.weaknesses.join(', ') : (Array.isArray(item.focusAreas) ? item.focusAreas.join(', ') : '');
      article.innerHTML = `
        <div class="attempt-top">
          <strong>${safeHTML(item.userName || item.employeeCode || '-')}</strong>
          <span class="status-pill">${Number(item.pct || 0)}%</span>
        </div>
        <div class="mini-meta">${safeHTML(item.employeeCode || '-')} · ${safeHTML(scopeText)} · ${safeHTML(fmtDate(item.createdAt))}</div>
